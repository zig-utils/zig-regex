...
pub fn main(init: std.process.Init) !void {
    ...
    for (invalid_patterns) |pattern| {
        ...
        if (Regex.compile(init.gpa, pattern)) |regex_value| {
    ...
    if (Regex.compile(init.gpa, user_pattern)) |regex_value| {
        var regex = regex_value;
        defer regex.deinit();
    ...
    if (Regex.compile(init.gpa, problematic_pattern)) |regex_value| {
        var regex = regex_value;
        regex.deinit();
    } else |err| {
        ...
        if (Regex.compile(init.gpa, fixed_pattern)) |regex_value| {
            var regex = regex_value;
            defer regex.deinit();
