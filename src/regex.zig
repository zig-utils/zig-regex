...
pub const Regex = struct {
    ...
    /// `countMatchingLines` parallelized across CPU cores. The buffer is split at
    /// newline boundaries into one chunk per worker (each chunk is whole lines, so
    /// per-line semantics are preserved and there is no double counting), every
    /// worker runs the single-threaded `countMatchingLines` on its chunk with its
    /// own thread-local DFA (the `Regex` is read only), and the partials are
    /// summed. Falls back to the serial path for small inputs, a single core, or
    /// any thread-spawn failure.
    pub fn countMatchingLinesParallel(self: *const Regex, input: []const u8) !usize {
        ...
        // Chunk boundaries: chunk i is [starts[i], ends[i]); the '\n' that
        // separates two chunks is consumed between them (start = prev end + 1).
        ...
        while (w < workers) : (w += 1) {
            if (w == workers - 1) {
                ...
                const nl = std.mem.indexOfScalarPos(u8, input, approx, '\n') orelse input.len;
                ends[w] = nl;
            }
            made = w + 1;
        ...
        var i: usize = 0;
        while (i < workers) : (i += 1) {
            ctxs[i] = .{ .re = self, .chunk = input[starts[i]..ends[i]] };
        }
        // Spawn workers 1..n; run worker 0 on this thread.
        i = 1;
        while (i < workers) : (i += 1) {
            threads[i] = std.Thread.spawn(.{}, Worker.run, .{&ctxs[i]}) catch null;
            if (threads[i] == null) {
                // Spawn failed: this chunk runs inline below alongside chunk 0.
            }
        }
        Worker.run(&ctxs[0]);
        var total: usize = 0;
        ...
        while (i < workers) : (i += 1) {
            if (threads[i]) |t| {
                ...
                Worker.run(&ctxs[i]); // ran inline (spawn had failed)
            }
            any_err = any_err or ctxs[i].err;
        }
        if (any_err) return self.countMatchingLines(input); // be safe on any worker error
        i = 0;
