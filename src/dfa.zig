...
pub const LazyDfa = struct {
    ...
    /// Eagerly materialize every reachable state's full 256-byte transition row,
    /// so the DFA holds the complete automaton and can then be shared read-only
    /// across threads (no lazy `computeStep` at scan time). Non-anchored search
    /// DFA only. Returns DfaOverflow if the complete DFA exceeds MAX_STATES.
    pub fn buildFull(self: *LazyDfa) Error!void {
        std.debug.assert(self.unanchored and !self.anchored);
        _ = try self.getStart(0);
        var idx: usize = 0;
        while (idx < self.keys.items.len) : (idx += 1) {
            var c: usize = 0;
            while (c < 256) : (c += 1) {
                if (self.trans.items[idx * ALPHA + c] == UNCOMPUTED) {
                    _ = try self.computeStep(@intCast(idx), c);
                }
            }
        }
    }

    /// `countMatchingLines` over a fully-built DFA (see `buildFull`): `const`, and
    /// computes no transitions, so the same `*const LazyDfa` is safe to scan from
    /// many threads concurrently. Caller must have called `buildFull` first.
    pub fn countMatchingLinesShared(self: *const LazyDfa, input: []const u8) usize {
        std.debug.assert(self.unanchored and !self.anchored);
        const start_s: usize = @intCast(self.start_cache[0]); // populated by buildFull
        const start_accepts = self.acc_state.items[start_s];
        const trans = self.trans.items;
        var count: usize = 0;
        var ls: usize = 0;
        while (true) {
            const le = std.mem.indexOfScalarPos(u8, input, ls, '\n') orelse input.len;
            if (start_accepts) {
                ...
                var p = ls;
                while (p < le) {
                    const t = trans[s * ALPHA + input[p]]; // fully built: never UNCOMPUTED
                    s = @intCast(t & STATE_MASK);
                    p += 1;
                    if (t >= ACCEPT_BIT) {
                        count += 1;
